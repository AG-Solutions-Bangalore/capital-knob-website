...
export function BusinessesSection({ onEnquire }: BusinessesSectionProps) {
  ...
  // live from the `business-loan` category's sub-categories. Static
  // `businessesCards` stay only as the loading/empty fallback.
  const { data } = useCategoryQuery()
  const live = data?.data.find((c) => c.category_slug === 'business-loan')
  const cards =
    (live?.category_subs?.length ?? 0) > 0
      ? liveSubCards(live, data?.image_url, businessesCards)
      : businessesCards
